...
class VAE(Model):
    def __init__(self, args):
        ...
        # decoder: p(z1 | z2)
        modules = [NonLinear(self.args.z2_size, self.args.hidden_size, gated=self.args.gated, activation=nn.Tanh()),]
        for _ in range(0, self.args.num_layers - 1):
            modules.append(NonLinear(self.args.hidden_size, self.args.hidden_size, gated=self.args.gated, activation=nn.Tanh()))
        self.p_z1_layers = nn.Sequential(*modules)
        ...
        self.p_x_layers_z1 = nn.Sequential(
            NonLinear(self.args.z1_size, self.args.hidden_size, gated=self.args.gated, activation=nn.Tanh())
        )
    ...
    def forward(self, x):
        ...
        #print("z2_q_mean shape",z2_q_mean.shape)
        #print("z2_q_logvar shape",z2_q_logvar.shape)
        z2_q = self.reparameterize(z2_q_mean, z2_q_logvar)
        #print("z2_q shape",z2_q.shape)
        # z1 ~ q(z1 | x, z2)
        z1_q_mean, z1_q_logvar = self.q_z1(x, z2_q)
        z1_q = self.reparameterize(z1_q_mean, z1_q_logvar)
        #print("z1_q_mean shape",z1_q_mean.shape)
        #print("z1_q_logvar shape",z1_q_logvar.shape)
