...
def log_Softmax(x, mean, average=False, dim=None):
    # softmax에 로그 취함 softmax 0~ 1 사이
    log_softmax = torch.nn.LogSoftmax(dim)
    # x_mean에 softmax 다 분류
    log_likelihood = log_softmax(mean) * x
    if average:
        return torch.mean( log_likelihood, dim )
    else:
        return torch.sum( log_likelihood, dim )
