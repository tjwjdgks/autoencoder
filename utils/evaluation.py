...
def evaluate_vae(args, model, train_loader, data_loader, epoch, dir, mode):
    ...
    def NDCG_binary_at_k_batch(X_pred, heldout_batch, k=100):
        ...
        # k개 만큼 작은 것 index 찾아냄 -> -X_pred 이모로 k개 만큼 큰 index 찾는 것
        idx_topk_part = bn.argpartition(-X_pred, k, axis=1)
        # X_pred X_pred에서 batch_users 별로  k개의 idx_topk_park index에 맞는 값들 뽑아냄
        topk_part = X_pred[np.arange(batch_users)[:, np.newaxis],
                           idx_topk_part[:, :k]]
        # -topk_park 이므로 다시 np.argsort로 내림차순 정렬 index
    ...
    def Recall_at_k_batch(X_pred, heldout_batch, k=100):
        ...
        # 예상 값들의 오름차순 순으로 k를 뽑아 True 로만듬 
        X_pred_binary[np.arange(batch_users)[:, np.newaxis], idx[:, :k]] = True
        # 0이상 값들 True로 만듬
        X_true_binary = torch.tensor((heldout_batch > 0), dtype=torch.float)
        # 예상 값과 실제 값의 교집합 수
        tmp = torch.tensor(np.logical_and(X_true_binary.cpu(), X_pred_binary), dtype=torch.float).sum(dim=1)
        #np.mininum ~~ 최대 1의 갯수
        recall = tmp / np.minimum(k, X_true_binary.cpu().sum(dim=1))
        return recall
