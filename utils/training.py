...
def train_vae(epoch, args, train_loader, model, optimizer):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if args.model_name == 'baseline2':
            if epoch%20 == 1:
                loss, RE, KL = model.calculate_loss(x, beta,average=True,epoch= epoch,batch_idx = batch_idx)
            else :
                loss, RE, KL = model.calculate_loss(x, beta, average=True,epoch = -1 , batch_idx = -1)
        else :
